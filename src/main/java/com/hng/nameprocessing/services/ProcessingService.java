...
public class ProcessingService {
  ...
  public CompletableFuture<ApiResponse> processName(String name) {
    ...
    CompletableFuture<NationalizeResponse> nationalizeFuture =
        nationalizeClient.nationalizeRequest(name);
    ...
    return CompletableFuture.allOf(agifyFuture, nationalizeFuture, genderizeFuture)
        .thenApply(
            ignored ->
                aggregateResponse(
                    agifyFuture.join(), nationalizeFuture.join(), genderizeFuture.join()));
  }

  private ApiResponse aggregateResponse(
            ...
            .orElseThrow(
                () ->
                    new ServiceValidationException(
                        "Nationalize returned an invalid response", 502));

    BigDecimal countryProbabilityBigDecimal =
        new BigDecimal(maxCountry.getProbability()).setScale(2, RoundingMode.FLOOR);
    ...
    return FreshResponseDto.builder().status("success").data(dataDto).build();
  }
