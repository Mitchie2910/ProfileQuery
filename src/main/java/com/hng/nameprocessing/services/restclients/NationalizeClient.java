...
public class NationalizeClient {
    ...
    Logger LOGGER = LoggerFactory.getLogger(NationalizeClient.class);

    public NationalizeClient(RestClient.Builder restClientBuilder,
    ...
    @SneakyThrows
    public CompletableFuture<NationalizeResponse> nationalizeRequest(String name) {
        LOGGER.info("NATIONALIZE STARTING");
        return CompletableFuture.supplyAsync(() -> restClient.get()
                ...
                .body(NationalizeResponse.class), executor);
    }
    ...
    private void errorHandler(HttpRequest request, ClientHttpResponse response) throws IOException {
        ...
        switch (response.getStatusCode().value()){
            case 400:
//                throw new MissingNameException(response.getStatusCode().value(), "Name parameter is missing");
            case 422:
