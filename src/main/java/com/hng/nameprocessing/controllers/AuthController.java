...
public class AuthController {
    ...
    @PostMapping("/token")
    public CompletableFuture<ResponseEntity<AuthResponse>> authenticate(@RequestBody LoginRequest request) {
        return cliAuth.authenticate(request.getCode(), request.getCodeVerifier())
                .thenApply(authResponse -> ResponseEntity
                        .status(HttpStatus.OK)
                        .body(authResponse));
    }

    @PostMapping("/refresh")
    public CompletableFuture<ResponseEntity<AuthResponse>> refresh(@RequestBody RefreshRequest request) {
        return cliAuth.refresh(request.getRefreshToken())
                .thenApply(authResponse -> ResponseEntity
                        .status(HttpStatus.OK)
                        .body(authResponse));
    }
